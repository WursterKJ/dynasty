...
tot_pts_max = round(user_select_df["points_uww"].sum(), 1)
avg_pts_max = round((user_select_df["points_uww"].sum())/17, 1)

avg_ppg = round(user_select_df["ppg_uww"].mean(), 1)
avg_ppg_qb = round(user_select_qb["ppg_uww"].mean(), 1)
avg_ppg_rb = round(user_select_rb["ppg_uww"].mean(), 1)
avg_ppg_wr = round(user_select_wr["ppg_uww"].mean(), 1)
avg_ppg_te = round(user_select_te["ppg_uww"].mean(), 1)
...
stat_table = user_select_df.sort_values("position").filter(items=["position", "full_name", "team", "gp", "points_uww", "ppg_uww"]).rename(columns={"position":"Position", "full_name":"Player", "team":"Team", "gp":"Games", "points_uww":"Points", "ppg_uww":"PPG"})
