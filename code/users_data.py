...
def users_refresh():
    ...
    users_df_filtered_freedom = users_df_raw_freedom.filter(items = ["user_id", "username", "display_name"])
    users_df_freedom = users_df_filtered_freedom.rename(columns={"user_id":"owner_id_freedom", "username":"username_freedom", "display_name":"display_name_freedom"}).astype({"owner_id_freedom": str})
    users_df_filtered_uww = users_df_raw_uww.filter(items = ["user_id", "username", "display_name"])
    users_df_uww = users_df_filtered_uww.rename(columns={"user_id":"owner_id_uww", "username":"username_uww", "display_name":"display_name_uww"}).astype({"owner_id_uww": str})
